...
p7.createSignedData = function() {
  ...
  function verifySignerInfos(mds, caStore, options) {
    ...
    if(!content) {
      throw new Error(
        'Could not verify PKCS#7 message; there is no content to verify.');
    }

    if(msg.signers.length === 0) {
      throw new Error('There are no signatures to verify.');
    }
    ...
    for(var i = 0; i < msg.signers.length; ++i) {
      ...
      if(signer.authenticatedAttributes.length === 0) {
        ...
        if(!foundDigest) {
          throw new Error('Authenticated attributes missing digest!  Unable to verify signature.');
        }
