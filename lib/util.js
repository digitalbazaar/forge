 ...
 *
 * @param input {string} binary encoded string of bytes to base64-encode.
 *
 * @return the encoded value.
 */
util.encodeBase64Url = function encodeBase64Url(input) {
  const enc = util.encode64(input);
  return enc
    .replace(/=/g, '')
    .replace(/\+/g, '-')
    .replace(/\//g, '_');
 ...
 * trailing '=', line breaks, whitespace, and other characters are omitted.
 *
 * @param input {string} the base64-encoded input to decode.
 *
 * @return the binary encoded string.
 */
util.decodeBase64Url = function decodeBase64Url(input) {
  ...
  return util.decode64(normalInput);
};
