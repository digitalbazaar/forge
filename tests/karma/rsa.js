...
describe('web worker rsa', function() {
  it('should generate key pairs when running forge in a web worker', function(done) {
    ...
    testWorker.addEventListener('message', function (event) {
      ...
      // This is primarily to ensure that the node-forge code runs successfully
      // in the worker (e.g. no usages of `window` or similar). So, comparing
      // for structural sanity of the result only should be fine
